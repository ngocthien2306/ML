...
path = 'C:/Users/hp/Downloads/HK2 21-22/Machine Learning/CodeEx/code_week3/datasets'
def ReadData(path):
    return pd.read_excel(path)

raw_data = ReadData(path + '/NguyenNgocThien_19110148_Week03_Data.xlsx')
...
def Show_Data_OneChart_Scatter(raw_data, x, y):
    ...
    plt.savefig('code_week3/figures/scatter_1_feat.png', format='png', dpi=300)
    plt.show()      
...
def Show_Data_ManyCharts_Fix(raw_data, features):
    ...
    plt.savefig('code_week3/figures/scatter_mat_all_feat.png', format='png', dpi=300)
    plt.show()
...
raw_data.drop(columns = [], inplace=True) 
...
num_feat_names = ['Age', 'Value', 'Contract Years Left', "MP", "Total Mins/90", "Gls", "Ast", "(G+A)/90",] # =list(train_set.select_dtypes(include=[np.number]))
cat_feat_names = ['Player', 'Club', 'Position', "League"] # =list(train_set.select_dtypes(exclude=[np.number])) 
...
print('''
Lý do về dữ liệu làm giảm độ chính xác:
    + Feature có thể bị sai (một số samples có thể không phải căn hộ chung cư, diện tích, số phòng... sai)
    + Label (giá nhà) thay đổi theo cò, thời điểm (cùng 1 căn hộ có nhiều giá)
    + Dữ liệu có nhiễu (abnormal data: như những căn vài trăm m2)
    + Dữ liệu ít: có thể bớt feature (HƯỚNG, QUẬN HUYỆN), hoặc tăng thêm samples
